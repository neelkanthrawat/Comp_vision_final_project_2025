...
class RepLoRAGenerator(nn.Module):
    ...
    def forward(self):
        ...
        if self.kind == "A":
            A_q = raw_q.view(self.r, self.dim)   # shape: (r, dim)
            A_v = raw_v.view(self.r, self.dim)   # shape: (r, dim)
            return A_q, A_v
        else: # This is for B
            B_q = raw_q.view(self.dim, self.r)   # shape: (dim, r)
            B_v = raw_v.view(self.dim, self.r)   # shape: (dim, r)
            return B_q, B_v
